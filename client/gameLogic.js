...
let { Deck, equipments, monsters, shuffle, doors, treasures, decks, allCards } = require('./cards');
...
const appendMethods = {
	game: (game) => {
		...
		game.lootRoom = () => {
			...
			log(game.players[game.turn].hand.map((card) => card.name));
			openSnackbar(
				`You looted the ${game.players[game.turn].hand[game.players[game.turn].hand.length - 1].name} card.`
			);
			game.phase = 3;
		};

		game.endTurn = () => {
			if (game.players[game.turn].hand.length > game.players[game.turn].maxInventory) {
				openSnackbar(
					`You are carrying too many items! Please discard or use ${game.players[game.turn].hand.length -
	...
	battle: (battle) => {
		...
		battle.buffs.getTotal = (side) => {
			return battle.buffs[side].map((buff) => buff.bonus).reduce((num1, num2) => num1 + num2, 0);
		};
		...
		battle.resolve = () => {
			...
			if (battle.getPlayers().some((player) => player.class && player.class.name === 'Warrior')) {
				playerTotal++;
			}
		...
		battle.end = () => {
			...
			battle.players.forEach((playerIdx) => (battle.game.players[playerIdx].inBattle = false));
	...
	player: (player) => {
		...
		player.checkRequirements = () => {
			...
			while (i < player.allEquips.length) {
				...
				if (item.requirement && !item.requirement(player) && (!player.hireling || item !== player.hireling.equipment)) {
					player.unequip(item);
				} else i++;
			}
		};
		...
		player.unequip = (item) => {
			...
			switch (item.type) {
				case 'Equipment':
					...
					if (bodyPart === 'hands' || bodyPart === 'misc') {
						player.equipment[bodyPart].splice(player.equipment[bodyPart].indexOf(item), 1);
						player.equipment.numHands -= item.numHands;
					} else {
			...
			if (player.hireling && item === player.hireling.equipment) player.hireling.equipment = null;
		...
		player.cast = (cardIdx, target) => {
			const card = player.hand[cardIdx];
			if (card.type === 'Spell' || card.type === 'Curse' || card.type === 'Boost' || card.type === 'Buff') {
				card.effect(target);
				card.discard();
				player.hand.splice(cardIdx, 1);
...
class Game {
	constructor(playerNames) {
		...
		this.players = shuffle(playerNames.map((playerName) => appendMethods.player(new Player(playerName, 'Male', this))));
