...
class Game {
	constructor(playerNames) {
		...
		this.players = shuffle(playerNames.map((playerName) => new Player(playerName, 'Male', this)));
...
class Battle {
	constructor(monster, game) {
		...
		this.buffs = {
			...
			monster: [],
			getTotal: (side) => {
				return this.buffs[side].map((buff) => buff.bonus).reduce((num1, num2) => num1 + num2, 0);
			}
		};
		this.isActive = true;
	...
	resolve() {
		if (this.players.some((player) => player.class && player.class.name === 'Warrior')) {
			this.playerTotal++;
		}
