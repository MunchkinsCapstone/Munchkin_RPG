...
let { Deck, equipments, monsters, shuffle, doors, treasures, decks, allCards, allCardsObj } = require('./cards');
...
const appendMethods = {
	...
	battle: (battle) => {
		...
		battle.buffs.getTotal = (side) => {
			let attack = battle.buffs[side].map((buff) => buff.bonus).reduce((total, num) => total + num, 0);
	...
	player: (player) => {
		...
		player.unequip = (item) => {
			...
			if (player.hireling && item.id === player.hireling.equipment.id) player.hireling.equipment = null;
	...
	constructor(playerNames) {
		...
		this.players = shuffle(
			playerNames.map((playerName, index) =>
				appendMethods.player(new Player(playerName, [ 'Male', 'Female' ][Math.round(Math.random())], this, index))
			)
		);
